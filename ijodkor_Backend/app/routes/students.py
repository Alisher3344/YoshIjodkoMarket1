...
async def all_students_grouped_by_school(
    ...
    # User ma'lumotini ham olish: telegram_username uchun
    # User ni admin_id (admin yaratgan) yoki user_id (telegram) orqali bog'laymiz
    students_res = await db.execute(
        select(Student).order_by(Student.id.desc())
    )
    students = students_res.scalars().all()

    # Foydalanuvchilarni ID bo'yicha bir martalik yuklash (telegram_username uchun)
    user_ids = set()
    for s in students:
        if s.user_id:
    ...
    for student in students:
        # 'rejected' o'quvchilarni superadmin ham ko'rmasligi kerak (yashirin)
