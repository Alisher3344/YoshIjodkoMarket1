...
import { ArrowRight, Shield, Truck, CreditCard, Award, Star } from 'lucide-react';
import { useNavigate } from 'react-router-dom';
import useStore from '../store/useStore';
import { products as seedProducts, featuredCategories } from '../data/products';
import { categoryLabels } from '../data/translations';
import ProductCard from '../components/product/ProductCard';

export default function HomePage() {
  const { t, lang, setSelectedCategory, products: apiProducts, fetchProducts } = useStore();
  ...
  const hitProds = [...allProds].filter(p => p.badge === 'hit' || p.sold > 10).slice(0, 8);
  const displayProds = tab === 'new' ? newProds : hitProds;
  const handleCat = (key) => { setSelectedCategory(key); navigate('/catalog'); };

  const features = [
    { icon: <Truck size={22}/>, titleUz: "Bepul yetkazib berish", titleRu: "Бесплатная доставка", descUz: "100 000 so'mdan yuqori buyurtmalarda", descRu: "При заказе от 100 000 сум" },
    { icon: <Shield size={22}/>, titleUz: "Kafolat beriladi", titleRu: "Гарантия качества", descUz: "Har bir mahsulot sifatli", descRu: "Каждый товар качественный" },
    { icon: <CreditCard size={22}/>, titleUz: "Onlayn to'lov", titleRu: "Онлайн оплата", descUz: "Click, Payme, Uzum", descRu: "Click, Payme, Uzum" },
    { icon: <Award size={22}/>, titleUz: "O'quvchilar ijodi", titleRu: "Творчество учеников", descUz: "100% qo'lda yasalgan", descRu: "100% ручная работа" },
  ];
    ...
    <div className="min-h-screen">
      ...
      <section className="max-w-7xl mx-auto px-4 py-4">
        <div className="grid grid-cols-1 lg:grid-cols-4 gap-3">
          ...
          <div className="lg:col-span-3 relative rounded-2xl overflow-hidden" style={{minHeight: 320}}>
            <img
              src="/hero-banner.png"
            ...
            <div className="relative z-10 p-8 md:p-12 flex flex-col justify-center" style={{minHeight: 320}}>
              <div className="flex items-center gap-2 mb-4">
                <span className="bg-[#f97316] text-white text-xs font-black px-3 py-1.5 rounded-full flex items-center gap-1.5 w-fit">
                  ...
                  {lang === 'uz' ? "Maxsus ehtiyojli bolalar ijodi" : "Творчество детей с особыми потребностями"}
                </span>
              </div>
              ...
              <div className="flex flex-wrap gap-3">
                ...
                >
                  {lang === 'uz' ? "Ijod asarlarini ko'rish" : "Смотреть работы"} <ArrowRight size={16}/>
                </button>
                <button
              ...
              <div className="flex gap-6 mt-7">
                {[
                  ...
                  <div key={i}>
                    ...
                    <div className="text-white/60 text-xs">{lang === 'uz' ? s.labelUz : s.labelRu}</div>
                  </div>
                ))}
              </div>
            </div>
          ...
          <div className="flex flex-col gap-3">
            ...
            <div className="bg-gradient-to-br from-rose-500 to-pink-600 rounded-2xl p-4 text-white text-center">
              ...
              <div className="font-black text-sm leading-tight">
                {lang === 'uz' ? "Imkoniyati cheklangan o'quvchilar reytingi" : "Рейтинг учеников с ограниченными возможностями"}
              </div>
              <div className="text-white/70 text-[10px] mt-1">
                {lang === 'uz' ? "O'quv yili reytingi" : "Рейтинг учебного года"}
              </div>
            </div>

            {/* Top 3 */}
            {(() => {
              const disabledProds = allProds.filter(p => p.studentType === 'disabled');
              ...
              disabledProds.forEach(p => {
                ...
                if (!authorMap[key]) {
                  authorMap[key] = {
                    author: p.author, authorRu: p.authorRu || p.author,
                    school: p.school, schoolRu: p.schoolRu || p.school,
                    photo: p.photo,
                    totalSold: 0, totalRevenue: 0, rating: p.rating,
                  };
                }
                authorMap[key].totalSold += (p.sold || 0);
                authorMap[key].totalRevenue += (p.sold || 0) * p.price;
              });
              const ranked = Object.values(authorMap).sort((a, b) => b.totalRevenue - a.totalRevenue).slice(0, 3);
              const medals = ['🥇', '🥈', '🥉'];
              const medalColors = [
              ...
              return ranked.map((s, i) => (
                <div key={i} className={`${medalColors[i]} rounded-2xl p-3 text-white flex items-center gap-3`}>
                  {/* Rasm — katta, chap tomonda */}
                  {s.photo ? (
                    <img src={s.photo} alt={s.author} className="w-14 h-14 rounded-xl object-cover border-2 border-white/60 flex-shrink-0"/>
                  ) : (
                    <div className="w-14 h-14 rounded-xl bg-white/20 flex items-center justify-center text-2xl flex-shrink-0">👤</div>
                  )}
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <div className="text-white/75 text-[10px] truncate">{lang === 'uz' ? s.school : s.schoolRu}</div>
      ...
      <section className="max-w-7xl mx-auto px-4 py-8">
        <div className="flex items-center justify-between mb-5">
          <h2 className="text-xl font-black text-gray-800">{t('categories')}</h2>
          <button onClick={() => navigate('/catalog')} className="text-[#1a56db] text-sm font-semibold flex items-center gap-1 hover:underline">
      ...
      <section className="max-w-7xl mx-auto px-4 pb-8">
        <div className="flex items-center justify-between mb-5">
          <div className="flex gap-2 bg-gray-100 rounded-xl p-1">
            <button
              onClick={() => setTab('new')}
              className={`px-4 py-2 rounded-lg text-sm font-bold transition ${tab === 'new' ? 'bg-white shadow text-[#1a56db]' : 'text-gray-500'}`}
            >
              {t('newProducts')}
            </button>
            <button
              onClick={() => setTab('hit')}
              className={`px-4 py-2 rounded-lg text-sm font-bold transition ${tab === 'hit' ? 'bg-white shadow text-[#1a56db]' : 'text-gray-500'}`}
            >
              {t('popularProducts')}
            </button>
          </div>
